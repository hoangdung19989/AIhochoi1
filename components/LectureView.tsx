...
const LectureView: React.FC<LectureViewProps> = ({ course, onExit }) => {
  ...
  const handleSelectLesson = (lesson: Lesson) => {
    if (lesson.type === 'video' && lesson.videoUrl) {
        ...
        setMarkedCompleted(false); // Reset status for new lesson
    } else {
    ...
    <div className="h-screen w-full bg-brand-bg flex overflow-x-hidden">
        <main className="flex-1 p-6 flex flex-col">
          <div className="flex-grow flex flex-col">
            ...
            <div className="flex-shrink-0 p-6 bg-white rounded-lg border border-slate-200 shadow-sm flex justify-between items-start">
              <div>
                 <h2 className="text-2xl font-bold mb-2 text-slate-800">{currentLesson?.title || 'Chưa chọn bài học'}</h2>
                 <p className="text-slate-600">Video bài giảng được tổng hợp từ nguồn học liệu mở.</p>
              </div>
              ...
              {user && currentLesson && (
                  ...
                  >
                      ...
                      {markedCompleted ? 'Đã hoàn thành' : 'Đánh dấu hoàn thành'}
                  </button>
              )}
            </div>
          </div>
