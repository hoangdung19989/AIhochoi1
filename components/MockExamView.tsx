...
const LoadingSpinner: React.FC<{ name: string }> = ({ name }) => (
  <div className="flex flex-col items-center justify-center text-center p-8">
    ...
    <p className="text-slate-600 font-semibold">Đang tạo đề thi thử môn {name}...</p>
    <p className="text-slate-500 text-sm">AI (Gemini) đang chuẩn bị cả trắc nghiệm và tự luận.</p>
  </div>
);
    ...
    return (
        <div className="mt-8 text-left">
            <h3 className="text-xl font-bold text-slate-800 mb-4 flex items-center">
                <PencilSquareIcon className="h-6 w-6 mr-2 text-amber-600"/>
                Phần Tự Luận
            </h3>
                ...
                {questions.map((item, index) => (
                    <div key={index} className="bg-slate-50 rounded-xl p-5 border border-slate-200">
                        ...
                        <button 
                            ...
                            className="text-sm font-semibold text-amber-600 hover:text-amber-800 focus:outline-none underline decoration-dotted"
                        >
...
const MockExamView: React.FC<MockExamViewProps> = ({ subject, grade, onBack, onBackToSubjects }) => {
  ...
  const fetchMockExam = useCallback(async () => {
    setIsLoading(true);
    setError(null);
    setQuiz(null);
    ...
    try {
      const quizData = await generateMockExam(subject.name, grade.name);
      setQuiz(quizData);
      
       // Parse time limit (usually 60-90 minutes for mock exams)
      const timeString = quizData.timeLimit;
      const minutes = parseInt(timeString.match(/\d+/)?.[0] || '60'); 
  ...
  const handleAnswerSelect = async (option: string) => {
    ...
    // --- PHASE 1: Ghi nhận câu trả lời vào Database ---
    if (user) {
        ...
        try {
            ...
            Object.assign(payload, { grade: grade.name, exam_type: 'mock' });

            const { error } = await supabase.from('question_attempts').insert(payload);
            if (error) {
                 console.error("Failed to log question attempt (Mock):", error.message);
            }
        } catch (err) {
  ...
  // Effect to save result when quiz is finished
  useEffect(() => {
    if (isQuizFinished && user && quiz) {
        const saveResult = async () => {
            try {
                ...
                Object.assign(payload, { exam_type: 'mock' });

                const { error } = await supabase.from('exam_results').insert(payload);
                if (error) {
                    console.error('Error saving mock exam result:', error.message);
                } else {
                    console.log('Mock exam result saved successfully');
                }
            } catch (err) {
                console.error('Exception saving mock exam result:', err);
            }
        };
        saveResult();
    }
  }, [isQuizFinished, user, quiz, score, subject.name, grade.name]);


  const renderContent = () => {
    if (isLoading) {
      return <LoadingSpinner name={`${subject.name} ${grade.name}`} />;
    }
    if (error) {
      return (
        <div className="text-center text-red-600 bg-red-50 p-6 rounded-lg">
          ...
          <p className="mt-2">{error}</p>
           <button onClick={fetchMockExam} className="mt-4 px-4 py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700">
            Thử lại
          </button>
        </div>
      );
    ...
    if (isQuizFinished) {
      return (
        <div className="text-center p-6 sm:p-8">
          <h3 className="text-2xl font-bold text-slate-800 mb-2">Hoàn thành bài thi thử!</h3>
          <p className="text-lg text-slate-600 mb-6">
            Bạn đã trả lời đúng <span className="font-bold text-amber-600">{score}</span> / {quiz.questions.length} câu trắc nghiệm.
          </p>
          {user && <p className="text-sm text-green-600 mb-6 font-medium">Kết quả thi đã được lưu vào hệ thống.</p>}
          ...
          <div className="mt-10 flex justify-center space-x-4">
            ...
            <button onClick={fetchMockExam} className="px-5 py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700">
              Làm bài thi khác
            </button>
          </div>
        </div>
    ...
    if (currentQuestion) {
      const isTimeLow = timeLeft < 300; // Warning if less than 5 mins
      ...
      return (
        <div className="p-6 relative">
            ...
            <div className="text-center mb-4 border-b border-slate-100 pb-4">
                <p className="text-sm text-slate-500">Đề được tạo dựa trên tài liệu của:</p>
                <p className="font-semibold text-slate-700">{quiz?.sourceSchool}</p>
            </div>
          ...
          <div className="w-full bg-slate-200 rounded-full h-2 mb-6">
             <div className="bg-amber-500 h-2 rounded-full transition-all duration-300" style={{ width: `${((currentQuestionIndex + 1) / (quiz?.questions.length || 1)) * 100}%` }}></div>
          </div>
          ...
          {timeLeft === 0 && !isAnswered && (
               <div className="mt-6 p-4 bg-red-50 border border-red-200 rounded-lg text-center">
                   ...
                   <button 
                        onClick={() => setCurrentQuestionIndex(quiz?.questions.length || 0)} 
        ...
        <div className={`fixed bottom-0 left-0 right-0 p-4 sm:p-6 z-10 animate-slide-in-bottom`}>
            <div className={`max-w-4xl mx-auto p-5 rounded-xl shadow-2xl border-t-4 ${isCorrect ? 'bg-green-50 border-green-500' : 'bg-red-50 border-red-500'}`}>
                <div className="flex items-start">
                    ...
                    >
                       {currentQuestionIndex === (quiz?.questions.length || 0) - 1 ? 'Xem kết quả & Tự luận' : 'Câu tiếp theo'}
                    </button>
                </div>
            </div>
        </div>
  ...
  return (
     <div className="container mx-auto max-w-4xl relative">
      <div className="flex items-center text-sm text-slate-500 mb-4 flex-wrap">
        ...
        <button onClick={onBackToSubjects} className="hover:underline">Thi thử</button>
        <ChevronRightIcon className="h-4 w-4 mx-1" />
