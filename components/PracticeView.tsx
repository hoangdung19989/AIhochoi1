...
const PracticeView: React.FC<PracticeViewProps> = ({ 
  ...
  useEffect(() => {
      if (quizData && !isLoading) {
          ...
          // FIX: Changed String(defaultTime) to defaultTime.toString() to fix "not callable" error.
          const minutes = parseInt(timeString.match(/\d+/)?.[0] || defaultTime.toString()); 
  ...
  // Timer logic
  useEffect(() => {
    // FIX: Used ReturnType<typeof setInterval> for browser compatibility instead of NodeJS.Timeout.
  ...
  const handleAnswerSelect = async (option: string) => {
    ...
    if (user) {
        ...
        try {
            ...
            Object.assign(payload, { grade: grade.name, exam_type: 'practice' });

            const { error } = await supabase.from('question_attempts').insert(payload);
            if (error) {
                console.error("Failed to log question attempt (Practice):", error.message);
            }
        } catch (err) {
