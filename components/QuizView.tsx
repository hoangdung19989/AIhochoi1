...
const QuizView: React.FC<QuizViewProps> = ({ subject, grade, testType, onBack, onBackToSubjects }) => {
  ...
  const handleAnswerSelect = async (option: string) => {
    ...
    if (user) {
        ...
        try {
            // Updated to include grade and exam_type if columns exist
            const payload: any = {
                user_id: user.id,
            ...
            // Try to add these if columns are created
            // We use spread/any to avoid TS error if types aren't fully updated yet for the table definition
            Object.assign(payload, { grade: grade.name, exam_type: 'test' });

            const { error } = await supabase.from('question_attempts').insert(payload);
            if (error) {
                // Fail silently or log, don't block user
                console.error("Failed to log question attempt (DB might miss columns):", error.message);
            }
        } catch (err) {
    ...
    if (isQuizFinished && user && quiz) {
        const saveResult = async () => {
            try {
                ...
                // Add exam_type for separation
                Object.assign(payload, { exam_type: 'test' });
                ...
                console.error('Exception saving result:', err);
            }
        };
        saveResult();
    }
    ...
    if (isQuizFinished) {
      return (
        <div className="text-center p-6 sm:p-8">
          ...
          {user && <p className="text-sm text-green-600 mb-6 font-medium">Kết quả đang được lưu vào hệ thống...</p>}
